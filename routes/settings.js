...
User.find((err, users) => {
  ...
  Settings.findOne((err, settings)=>{
    ...
    res.render('settingsupdate', {settings: settings, users: users, layout: 'layoutadmin'})
...
router.post('/settingsupdate', (req, res, next) => {
  ...
  User.find((err, users) => {
      ...
      Settings.findOneAndUpdate(id, settings, {new: true}, (err, settings) => {
        ...
        res.render('settingsupdate', {layout: 'layoutadmin', users: users, settings: settings, msg: '<h5 class="text-left btn-success rounded py-2 pl-4 my-2 mr-1">Business Settings Updated!</h5>'})
